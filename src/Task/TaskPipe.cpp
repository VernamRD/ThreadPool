...
std::shared_ptr<threadpool::TaskHandle> threadpool::TaskPipe::add_task(threadpool::ETaskPriority task_priority, std::unique_ptr<threadpool::Task>&& task)
{
...
void threadpool::TaskPipe::do_job_until_task_exists()
{
    TaskContext context;
...
void threadpool::TaskPipe::set_context(const TaskContext& context)
{
    task_context = context;
}

threadpool::TaskPtr threadpool::TaskPipe::pop_relevant_task(TaskContext& context)
{
    std::unique_lock lock(mutex);
    ...
    for (uint8_t priority = std::numeric_limits<uint8_t>::min(); priority <= static_cast<uint8_t>(ETaskPriority::Lowest); ++priority)
    {
        ...
        if (tasks.contains(current_priority) && tasks[current_priority].size() > 0)
        {
